...
/**
 * The MainFileChooser class extends JFileChooser and
 * provides customized file chooser dialog for the main frame.
 */
public class MainFileChooser extends JFileChooser {
  ...
  /**
   * Constructs a MainFileChooser object with the specified main frame.
   *
   * @param frame the main frame to associate with the file chooser
   */
  public MainFileChooser(MainFrame frame) {
    this.frame = frame;
    FileNameExtensionFilter filter
          = new FileNameExtensionFilter("Program Files", Settings.SAVE_FILE_EXTENSION);
    setFileFilter(filter);
    setAcceptAllFileFilterUsed(false);
    setCurrentDirectory(Settings.SAVE_DIRECTORY);
